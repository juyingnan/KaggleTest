...
for index in random.sample(range(len(data['data'])), img_count):
    ...
    y.append(np.array(data['labels'].iloc[index]))
plt.imshow(x[2])
plt.show()
...
print('Accuracy of the network on the 3200 test images: %d %%' % (100 * correct / total))

class_correct = list(0. for i in range(num_classes))
class_total = list(0. for i in range(num_classes))

savepath = r'C:\Users\bunny\Desktop\plane_test'
for i in [0, 1]:
    for j in [0, 1]:
